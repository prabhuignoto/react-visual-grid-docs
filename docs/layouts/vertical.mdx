...
A vertical layout is the default layout of the Grid. A vertical layout will have a fixed number of columns and a vertically scrollable grid container.
...
> The `gridLayout` property can be set to `vertical` or `horizontal` to enable the respective layout.
